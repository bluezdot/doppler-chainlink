...
function DeployDoppler() {
  ...
  const { tokenFactory, governanceFactory, v3Initializer, liquidityMigrator, airlock } = addresses;
  ...
  const handleMaxShareToBeSoldChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setMaxShareToBeSold(e.target.value);
  };

  const handleMaxShareToBondChange = (e: React.ChangeEvent<HTMLInputElement>) => {
